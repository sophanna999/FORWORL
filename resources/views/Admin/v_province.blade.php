        ...
        <div class="modal-content">
            <form id="FormAdd">
                ...
                <div class="form-group">
                    ...
                    <select name="country_id" class="select2 form-control" tabindex="-1" data-placeholder="Select country" id="add_country_id" required="" >
                        <option value="">Select country</option>
                        @foreach($Countriess as $Countries)
                        <option value="{{$Countries->id}}">{{$Countries->code}}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="modal-content">
            ...
            <form id="FormEdit">
                ...
                <div class="form-group">
                    ...
                    <select name="country_id" data-placeholder="Select country" tabindex="-1" class="select2 form-control" id="edit_country_id" required="" >
                        <option value="">Select country</option>
                        @foreach($Countriess as $Countries)
                        <option value="{{$Countries->id}}">{{$Countries->code}}</option>
                        @endforeach
...
<script>
     ...
     var TableList = $('#TableList').dataTable({
        "ajax": {
            "url": url_gb+"/admin/Province/Lists",
            "data": function ( d ) {
    ...
    $('body').on('click','.btn-edit',function(data){
        ...
        $.ajax({
            method : "GET",
            url : url_gb+"/admin/Province/"+id,
            dataType : 'json'
        }).done(function(rec){
            $('#edit_country_id').val(rec.country_id).trigger("change");
    ...
    $('body').on('click','.btn-delete',function(e){
        ...
        swal({
            ...
            $.ajax({
                method : "POST",
                url : url_gb+"/admin/Province/Delete/"+id,
                data : {ID : id}
            }).done(function(rec){
                if(rec.status==1){
                    swal(rec.title,rec.content,"success");
